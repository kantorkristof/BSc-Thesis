...
class Predictor(torch.nn.Module):
    def __init__(self, in_channels, hid_channels=64, num_outputs=1, conv_unit=2):
        ...
        self.non_causal_mlp = nn.Sequential(
            nn.Linear(hid_channels, 2*hid_channels),
            nn.ReLU(),
            nn.Dropout(0.1),
            nn.Linear(2*hid_channels, hid_channels),
    ...
    def forward(self, x, edge_index, edge_attr, batch):
        ...
        out = self.get_causal_pred(graph_x)
        return out
    ...
    def get_non_causal_pred(self, graph_x):
        return self.non_causal_mlp(graph_x)
...
def split_graph(data, edge_score, ratio):
    ...
    for edge_index, N, C in zip(edge_indices, num_edges, cum_edges):
        ...
        non_causal_edge_weight = torch.cat([non_causal_edge_weight, single_mask[idx_drop]])
...
def compute_dir_loss(causal_out, non_causal_out, labels, alpha):
    ...
    dir_penalty = compute_interventional_risk_variance(causal_pred_list, non_causal_pred_list, labels)

    total_loss = causal_loss + alpha * dir_penalty

    return total_loss, causal_loss, dir_penalty, non_causal_loss
...
def main():
    ...
    parser.add_argument('--r', default=0.9, type=float, help='causal_ratio')
    ...
    for attribute_name in available_attributes:
        ...
        for seed in args.seed:
            ...
            att_net = Rationale_Generator(in_channels, args.r, args.channels).to(device)

            model_optimizer = torch.optim.Adam(
                list(g.parameters()) + list(att_net.parameters()),
                lr=args.net_lr
            )
            ...
            def test_metrics(loader, att_net, predictor):
                ...
                for graph in loader:
                    ...
                    (non_causal_x, non_causal_edge_index, non_causal_edge_attr, non_causal_edge_weight, non_causal_batch), edge_score = att_net(graph)
            ...
            for epoch in range(args.epoch):
                ...
                for graph in train_loader:
                    ...
                    total_loss, causal_loss, dir_penalty, non_causal_loss = compute_dir_loss(
                    ...
                    epoch_non_causal_loss += non_causal_loss.item()
                    num_batches += 1
                ...
                avg_non_causal_loss = epoch_non_causal_loss / num_batches
                ...
                logger.info(
                    ...
                    f"Non_Causal_Loss: {avg_non_causal_loss:.6f}"
                )
