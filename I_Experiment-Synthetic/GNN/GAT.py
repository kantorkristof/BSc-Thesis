...
class GAT(torch.nn.Module):
    def __init__(self, in_channels, hid_channels=64, num_outputs=1, heads=4):
        ...
        self.conv1 = GATConv(in_channels=hid_channels, out_channels=hid_channels//heads, 
                           heads=heads, concat=True, dropout=0.1)
        self.bn1 = BatchNorm(hid_channels) 
        self.relu1 = nn.ReLU()
        
        self.conv2 = GATConv(in_channels=hid_channels, out_channels=hid_channels, 
                           heads=heads, concat=False, dropout=0.1)
...
def main():
    ...
    parser.add_argument('--heads', default=8, type=int, help='GAT attention heads')
        ...
        for seed in args.seed:
            ...
            def test_metrics(loader, model):
                ...
                with torch.no_grad():
                    for graph in loader:
                        graph.to(device)
                        pred = model.predict(graph.x, graph.edge_index, graph.edge_attr, graph.batch)
                        correct += (pred.view(-1) == graph.y).sum().item()
            ...
            for epoch in range(args.epoch):
                ...
                for graph in train_loader:
                    ...
                    loss = F.binary_cross_entropy_with_logits(logits.view(-1), graph.y.float())
                ...
                val_mode()
                with torch.no_grad():
                    train_acc = test_metrics(train_loader, model)
                    val_acc = test_metrics(val_loader, model)
            ...
            with torch.no_grad():
                best_train_acc = test_metrics(train_loader, model)
                best_val_acc = test_metrics(val_loader, model)
                best_test_acc = test_metrics(test_loader, model)
